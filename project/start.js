...
const SHA256 = require('cryconst express = require('express');
const { MongoClient, ServerApiVersion } = require('mongodb');
const bodyParser= require('body-parser');
const app = express();
const session = require('express-session');
...
app.post("/modifygallery/:artworkid/:entry", async (req, res) => {
	...
	if(filenamepicture != ""){
		...
		let pfunc = '{"_gallery.'+entrynr+'._path"'+":"+'"'+displaypath+'"'+'}';

		let pathfunc = JSON.parse(pfunc);

		stevesDB.updateOne({"_gallery._artworkid": artid}, {$set: pathfunc});
	}
